...
function calculateHypotenuse(a, b) {
  ...
  const lengthHypotenuse = Math.sqrt((Math.pow(a, 2) + Math.pow(b, 2)));
...
function calculateVolumeCylinder(radius, height) {
  ...
  return Math.ceil(volumeCylinder);
...
function getFractionalPart(num) {
  ...
  const froundPart = Math.fround(fractionalPart);
  return froundPart;
...
function estimateInvestment(principal, interestRate, years) {
  ...
  const round = Math.round(sumInvest);
  return round;
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  const convertPrice = Math.fround(totalPrice, maxPrice);
